  ...
  return (
    <Link to={`/vehiculo/${v.slug}`} className="neu-card transition-transform duration-300 hover:-translate-y-2 cursor-pointer block">
      <div className="p-8 h-full flex flex-col" style={{ minHeight: 440 }}>
        ...
        <div
          className="rounded-3xl mb-6 overflow-hidden flex-grow"
          style={{ minHeight: 160, boxShadow: "inset 0 0 20px rgba(0,0,0,0.05)" }}
        >
          ...
          <div>
            {isEmployee && v.price_employee ? (
              <>
                ...
                <p className="text-2xl font-light">${fmt(v.price_employee)}</p>
                {v.is_public && (
              ...
              <>
                ...
                <p className="text-2xl font-light">${fmt(v.price_public)}</p>
              </>
            )}
          </div>
          <div>
